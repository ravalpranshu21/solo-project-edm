...
let html = `<div class="img-section">
  ...
  fetch('https://dummyjson.com/products/' + productId)
   ...
   .then(data => {
    ...
    newHTML = newHTML.replace('%newPrice%', (data.price * 100) - (data.price * data.discountPercentage / 100));
    ...
    newHTML = newHTML.replace('%price%', data.price * 100);
    ...
    (function (index) {
      fetch('https://dummyjson.com/products/' + cartItems[index])
       ...
       .then(data => {
        ...
        productPrice.textContent = " ₹" + (data.price)*80 + ".00";
          detailsContainer.appendChild(productPrice);
        total = total + (data.price)*80;
        console.log("total: "+total*80);
